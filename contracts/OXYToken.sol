...
contract OXYToken is Ownable, ERC1155 {
    ...
    event TokenCreated(uint indexed projectId, uint indexed tokenId, address indexed owner);

    // @dev Emited every time the smart contract owner issues new tokens
    event OxygenIssued(uint indexed tokenId, uint indexed amount, address indexed owner);

    /**
     * @notice Construct a new OXY Token
     * @dev ERC1155 receives as param the metadata address
     */
    constructor() ERC1155("https://gateway.pinata.cloud/ipfs/QmTgHC8XooNzaVQEUyCz36sYJjw9HETfs7kPpgPirRTfzm{id}") {
   ...
   /**
     ...
     */
    function createNewTokenBatch(uint256 _projectId) external onlyOwner returns (uint256) {

        require(_projectId != 0, "OXYToken::mint: Project ID has to be different than 0.");
        require(_projectId <= projectsCreated, "OXYToken::mint: Project has not been initialized yet.");
     ...
     */
    function mint(uint256 _tokenId, uint256 _amount, string memory _serialNumber) public onlyOwner {

        require(_tokenId != 0, "OXYToken::mint: Token ID has to be different than 0.");
        require(_tokenId <= tokenIds, "OXYToken::mint: Token has not been initialized yet.");
        require(tokenIdsToAmounts[_tokenId] == 0, "OXYToken::mint: '_tokenId' has already been minted.");
        bytes memory serialNumber = bytes(_serialNumber);
        require(serialNumber.length == 0,"OXYToken::mint: '_serialNumber' has to be different than empty.");
        require(serialNumbersToToken[serialNumber] == 0, "OXYToken::mint: '_serialNumber' has already been used.");
   ...
   /**
     ...
     */
    function release(address _account, uint256 _id, uint256 _amount) public virtual {
        require(_account == _msgSender() || isApprovedForAll(_account, _msgSender()),
   ...
   /**
     ...
     * - `_account` must have at least `amount` tokens of token type `id`.
     */
    function releaseBatch(address _account, uint256[] memory _ids, uint256[] memory _amounts) public virtual {
        require(_account == _msgSender() || isApprovedForAll(_account, _msgSender()),
